...
Cat=chromatic_data_extractor(astcat,'Survey',InPar.Survey);
astcatchrom=astcat;
for i=1:length(Cat)
    ...
    resvec=resvec';
    %(E - N -> N - E) since the paralactic angle rotation need to take the
    %north to the east
    ...
    q=PA;
    [respa, Rot]=Asmtry.Rotation2D(resvec,-q);
    ...
    correction_Dec_RA=Asmtry.Rotation2D(corr_pa_hat,q);
    ...
    Cat_chrom_corr(:,Cat(i).Col.ALPHAWIN_J2000)=Cat_chrom_corr(:,Cat(i).Col.ALPHAWIN_J2000)- ...
        (correction_Dec_RA(:,2)).*cos(Cat_chrom_corr(:,Cat(i).Col.DELTAWIN_J2000));
